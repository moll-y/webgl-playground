import vshader from "@glsl/5.1/shader.vert";
import fshader from "@glsl/5.1/shader.frag";
import { getWebGL2RenderingContext } from "@/utils";
import { Shader } from "@/shader";
import { mat4 } from "gl-matrix";

function main() {
  const gl = getWebGL2RenderingContext("5.1");
  ...
  const uTransformation = gl.getUniformLocation(
    shader.program,
    "u_transformations",
  );
  if (uTransformation === null) {
    throw new Error("Could not locate u_transformations.");
  }
  ...
  function render(timestamp) {
    ...
    mat4.fromTranslation(translation, [0.5, -0.5, 0]);

    const rotation = mat4.create();
    mat4.fromRotation(rotation, timestamp, [0, 0, 1]);

    const transformations = mat4.create();
    mat4.mul(transformations, translation, rotation);

    shader.use();
    gl.uniformMatrix4fv(uTransformation, false, transformations);
